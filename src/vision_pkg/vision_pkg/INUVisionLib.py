...
# # ==== ROS SERVICE HEADLESS DEBUG VIEW PATCH ====
# # ROS2 서비스 실행 중 matplotlib/Open3D 디버그 창이 X11 BadWindow로 죽는 문제 방지
...
#         def _safe_matplotlib_show(*args, **kwargs):
#             plt.close("all")

#         plt.show = _safe_matplotlib_show
...
#         def _safe_o3d_draw_geometries(*args, **kwargs):
#             return None

#         o3d.visualization.draw_geometries = _safe_o3d_draw_geometries

#     except Exception:
#         pass
# # ==== END ROS SERVICE HEADLESS DEBUG VIEW PATCH ====
...
#     #     "best.pt"
#     # )
#     MODEL_PATH = "/home/agxorin/integrated_ws/src/vision_pkg/vision_pkg/models/best.pt"
...
def estimate_pose_axes_from_obb3d(
    ...
    # X축: 가장 짧은 edge 방향 (4x2 그리퍼 단축 기준)
    edge_candidates.sort(key=lambda x: x[0], reverse=True)
    x_axis = normalize_vec(edge_candidates[0][1])
    ...
    if "4x2" in class_name_str or "2x4" in class_name_str:
        yaw_deg = normalize_yaw_deg_180(yaw_deg + 90.0)
...
def search_wide(color_rgb, depth, intrinsics, scale, V_visualize=True):
    ...
    #     "best.pt"
    # )
    MODEL_PATH = "/home/orinagx/integrated_ws/src/vision_pkg/vision_pkg/models/best.pt"
...
# 0-4. 바닥에서 1cm(0.01m) 이상 튀어나온 영역만 마스킹 (1cm 미만은 바닥으로 간주)
    ...
    if best_target is not None:
        ...
        if aspect_ratio >= 1.5:
            ...
            # 12시 방향(0, -1)을 기준으로 각도 계산
            # math.atan2(x축, y축)을 사용하여 12시 기준 좌/우 각도 도출
            yaw_deg = normalize_yaw_deg_180(math.degrees(math.atan2(vx, -vy)) + 90.0)
            # yaw_deg = yaw_deg
            ...
            # 12시 방향(0, -1)과 세로선 사이의 사이각을 0~90도로 절대값 출력
            yaw_deg = normalize_yaw_deg_180(math.degrees(math.atan2(abs(dx), abs(dy))) + 90.0)
            # yaw_deg =+ 90
