...
Description
-----------
Integrated dual-robot control node that initializes and manages
robot1 (10.0.2.7) and robot2 (10.0.2.8) simultaneously.
Provides step-based motion and home-return services for both cobots
based on joint configurations defined in ROBOT_CONFIGS.

Main Features
-------------
- Simultaneous initialization of two cobots with hardware speed multiplier
- /{robot_name}/robot_home     : Home position return service (Trigger)
- /{robot_name}/robot_move_step : Step-based motion command service (GetTargetPose)
- Joint (move_j) and linear (move_l_rel) motion handling
- L_VEL=1050 mm/s, L_ACC=2250 mm/s², speed_multiplier=2.0

Required Nodes
--------------
- rbpodo Cobot driver (physical robot connection)
- vision node : /get_target_pose (GetTargetPose)

Notes
-----
- All joint values in ROBOT_CONFIGS are in degrees
- move_j vel/acc is fixed at 255 (hardware maximum)
...
ROBOT_CONFIGS = {
    "robot1": {
        ...
        "separation_joint": [-91.02, 21.07, 26.68, 0.11, 131.03, -93.15],
        ...
        "drop_after_grip_joint": [-91.69, -57.11, 142.24, 9.11, 4.71, -12.0],   ###### 맵에서 속도 \줄여서 다시 측정
        # "drop_after_drop_joint": [-100.02, -16.63, 141.86, 6.93, -15.10, -10.03],



        ###  대회장
        "drop_joint": [-78.63, 23.09, 76.96, 0.0, 79.96, 14.21],
        "drop_joint2": [-100.0, 16.81, 85.07, 0.02, 78.14, -5.56],
        "drop_joint3": [-110.0, 19.98, 81.04, 0.02, 78.99, -13.60],
        "drop_joint4": [-79.38, 17.60, 84.07, 0.02, 78.34, 10.62],
        "drop_joint5": [-71.55, 20.77, 80.01, 0.02, 79.23, 18.45],

        "center_joint": [-108.2, -8.08, 112.77, 0.0, 75.32, -18.2],

        "drop_after_drop_joint": [-93.63, -12.14, 138.08, 12.87, -5.74, -13.72],
    ...
    "robot2": {
        ...
        ##대회용 
        "separation_joint": [-88.44, -12.95, 116.38, 5.74, -16.02, -5.99], ###### 맵에서 속도 \줄여서 다시 측정
        "drop_joint": [-98.13, 10.19, 134.32, -4.26, -46.87, 2.49],
        "drop_joint2": [-95.0, 9.82, 135.36, 0.0, -47.45, 0.0], # 2,3층 시퀀스만 해당
        "drop_joint3": [-95.0, 6.1, 135.25, 0.0, -43.63, 0.0], # 4층 시퀀스
        "drop_joint4": [-112.01, -2.04, 154.53, -14.49, -55.48, 6.69],  # 큰 나무 2층 콤보용 (캘리브레이션 필요)
...
class DualRobotNode(Node):
    def __init__(self):
        ...
        for robot_name, cfg in ROBOT_CONFIGS.items():
            ...
            robot.set_speed_multiplier(rc, 2.0)
