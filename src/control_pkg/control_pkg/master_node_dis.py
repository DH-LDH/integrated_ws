...
Description
-----------
Master node for the disassembly (RECYCLE) sequence.
Receives RECYCLE tasks from command_node and coordinates robot1 and robot2
to disassemble finished products using vision-guided pick operations.

Main Features
-------------
- Vision-based object detection and pose estimation for disassembly
- Cooperative dual-robot disassembly sequence (robot1 / robot2)
- Gripper control (open / grip) with settle-time management
- Y-axis scan retry logic for robust object detection
- Configurable motion parameters via ROS parameters
...
Notes
-----
- All tunable constants are defined at module level with _DEFAULT suffix
- time.sleep(0.3) in vision retry loop is intentional to prevent server overload
...
class BatteryDualDisassembly(Node):
    ...
    def run_estop_once(self):
        """
        E-stop 2층 분해.
          1단계: 2층 2x2 빨강 스캔 (z_extra=19.0) → 실제로는 1층 4x2 노랑 파지 → pos 체크
          2단계: 그리퍼 오픈 → Z -19mm 상승
                 (pos=300~400이면 YAW -90°와 동시 / pos=500~600이면 Z만)
                 → 2층 2x2 빨강 파지 → SEPARATION
        ...
        # ── 1단계: 2층 2x2 빨강 스캔 → 1층 4x2 노랑 위치로 접근 (z_extra=19.0) ──
        ...
        z_move     = (p.z * 1000.0 + self.Z_OFF) + 19.0   # z_extra=19.0 → 1층 4x2 노랑 깊이
        z_approach = z_move - self.Z_MARGIN
    ...
    def _run_new_seq_from_separation(
        self, name: str, top_label: str, mid_label: str, bot_label: str
    ) -> bool:
        """
        _run_new_seq 중간부터 실행 — robot1이 이미 top 블록을 잡고 SEPARATION 자세에 있을 때 사용.
        robot1: SEPARATION 출발 → pull_up → DROP → DROP_AFTER_HOME
        robot2: mid 그립 → 대기 → DROP
        robot1: bot 그립(DROP_AFTER_GRIP) → DROP_AFTER_DROP
        """
        self.get_logger().info(f"{name} 분해 재개 (robot1 SEPARATION 출발)")

        if not self.robot2_side_hold(mid_label):
            return False

        if not self.robot1_pull_up(top_label):
            self.robot2_drop_bottom_and_home(mid_label)
            return False
        ...
        self.get_logger().info(f"[DROP] robot1: {bot_label} → DROP_AFTER_DROP")
        if not self.send_pose(self.cli_r1, "DROP_AFTER_DROP"):
            return False
        self.set_gripper1(False)
    ...
    def run_hammer_once(self):
        return self._run_new_seq("망치", "4x2_blue", "3층 파랑", "2층 빨강", "1층 빨강", use_pos_check=True)

    def run_big_carrot_once(self):
        """
        큰 당근 4층 분해 시퀀스 (아이스크림과 동일한 로직).
          1단계: robot1 4층 초록 픽(pos체크) → robot2 1층 노랑 고정 → pull_up
          2단계: robot2 즉시 DROP4 → HOME
          3단계: robot1 CENTER 임시 배치 후 Z 이동 → YAW -90° → 3층 4x2 노랑 직접 픽 → SEPARATION2
        ...
        # ── 2단계: robot2 즉시 DROP4 → 그리퍼 오픈 → HOME ──────────
        self.get_logger().info("[DROP4] robot2: 1층 노랑 → DROP4 → HOME")
        if not self.send_pose(self.cli_r2, "DROP4"):
            self.get_logger().error("robot2: DROP4 실패")
            return False
        self.set_gripper(self.cli_g2, False)
        self.call(self.cli_h2, Trigger.Request())

        # ── 3단계: robot1 CENTER 임시 배치 → 3층 4x2 노랑 직접 픽 → SEPARATION2 ──
        self.get_logger().info("[CENTER] robot1: 4층 초록 내려놓기 → 3층 4x2 노랑 픽업")
        if not self.send_pose(self.cli_r1, "CENTER"):
            self.get_logger().error("robot1: CENTER 실패")
            return False
        self.sleep()
        ...
        self.move_z(self.cli_r1, self.CENTER_Z)
        req_yaw = GetTargetPose.Request()
        req_yaw.target_size = "YAW"
        req_yaw.yaw = -90.0
        self.call(self.cli_r1, req_yaw)
        if not self.set_gripper1(True):
            self.get_logger().error("[CENTER PICK] robot1 3층 4x2 노랑 파지 실패")
            return False

        if not self.send_pose(self.cli_r1, "SEPARATION2"):
            return False

        # ── 4단계: 나머지 3층 (3층 4x2 노랑 → 2층 2x2 노랑 → 1층 2x2 노랑) ──
        return self._run_new_seq_from_separation(
            "큰 당근 (하단 3층)",
            "3층 4x2 노랑",
            "2층 2x2 노랑",
            "1층 2x2 노랑",
    ...
    def run_burger_once(self):
        return self._run_new_seq("버거", "4x2_yellow", "3층 노랑", "4x2 빨강", "1층 노랑", use_pos_check=True)
    ...
    def run_ice_cream_once(self):
        ...
        # ── 3단계: robot1 CENTER 임시 배치 → 3층 초록 직접 픽 → SEPARATION ──
        self.get_logger().info("[CENTER] robot1: 4층 초록 내려놓기 → 3층 초록 픽업")
        if not self.send_pose(self.cli_r1, "CENTER"):
            self.get_logger().error("robot1: CENTER 실패")
            return False
        self.sleep()
        ...
        self.call(self.cli_r1, req_yaw)
        if not self.set_gripper1(True):
            self.get_logger().error("[CENTER PICK] robot1 3층 초록 파지 실패")
            return False

        if not self.send_pose(self.cli_r1, "SEPARATION2"):
            return False

        # ── 4단계: 나머지 3층 (3층 초록 → 2층 파랑+빨강 → 1층 노랑) ──
        return self._run_new_seq_from_separation(
            "아이스크림 (하단 3층)",
            "3층 2x2 초록",
            "2층 2x2 파랑, 2x2 빨강",
