...
#         self.cli_unlock_positions = self.create_client(Trigger, "/unlock_positions")
#         self.cli_g2 = self.create_client(SetBool, "/robot2/control_gripper")
...
#         )
#         self._khj_data: dict = {}
#         self._target_id_map_data: dict = {}
#         self.khj_valid: bool = False
#         self.precision_scan_failed: bool = False
#         self.khj_sub = self.create_subscription(
#             String, "/khj_point", self._khj_cb, 10
#         )
#         self.target_id_map_sub = self.create_subscription(
#             String, "/target_id_map", self._target_id_map_cb, 10
#         )
...
#     def update_floor_count_at_home(self, label, expected_count):
#         """
#         HOME으로 이동한 뒤 block_count를 읽어 current_floor_count_at_home을 갱신한다.
#         KHJ 유효 경로에서 사용: 카운트 검증 없이 읽은 값(또는 예상값)으로 기준을 업데이트한다.
#         """
#         home_res = self.call(self.cli_h, Trigger.Request())
#         if home_res is None or not home_res.success:
#             self.get_logger().error(f"[UPDATE_COUNT] {label}: HOME 이동 실패. 예상값 사용.")
#             after_count = expected_count
#         else:
#             after_time = time.monotonic()
#             time.sleep(0.1)
#             after_count = self.wait_decision_block_count(timeout_sec=5.0, after_time=after_time)
#             if after_count is None:
#                 self.get_logger().warn(
#                     f"[UPDATE_COUNT] {label}: block_count 수신 실패. 예상값={expected_count} 사용."
#                 )
#                 after_count = expected_count

#         self.get_logger().info(
#             f"[UPDATE_COUNT] {label}: after={after_count} (expected={expected_count}) → "
#             "KHJ 유효이므로 검증 없이 기준값 갱신"
#         )
#         self.current_floor_count_at_home = after_count
#         self.current_insert_start_count = after_count
#         self.current_insert_verify_after_time = None
...
#         # 새 명령 시작: 이전 동결 해제 → 최신 detections 수신 재개
#         self.get_logger().info("[INIT] birdseye 동결 해제 요청: /unlock_positions")
#         call_trigger_if_available(
#             self.cli_unlock_positions,
#             "/unlock_positions",
...
#         # 이전 명령의 stale KHJ 데이터를 제거하고 새 발행을 기다린다.
#         # _khj_data가 남아있으면 루프가 즉시 탈출해 stale 데이터로 khj_valid가 결정된다.
#         self._khj_data = {}
#         self.get_logger().info(
#             "[INIT] KHJ 데이터 갱신 대기 (최대 5s, khj_point 발행 주기 0.5s)"
#         )
#         deadline = time.monotonic() + 5.0
#         while time.monotonic() < deadline:
#             if self._target_id_map_data and self._khj_data:
#                 break
#             if getattr(self, "executor", None) is None:
...
#         vision_count = len(self._target_id_map_data)
#         khj_count = len(self._khj_data)
#         self.khj_valid = vision_count > 0 and vision_count == khj_count
#         self.get_logger().info(
#             f"[KHJ VALID] target_id_map={vision_count}개, "
#             f"khj_matched={khj_count}개 → khj_valid={self.khj_valid}"
#         )
...
#     def _target_id_map_cb(self, msg: String):
#         try:
#             self._target_id_map_data = json.loads(msg.data)
#         except Exception as e:
#             self.get_logger().warn(f"[TARGET_ID_MAP] 파싱 실패: {e}")
...
#             ],
#             key=lambda kv: float(kv[1].get("dist_cm", 999.0)),  # 가까운 것부터
#         )
...
#             else:
#                 self.get_logger().error(
#                     "[PRECISION] 정밀 재촬영 실패. 조립 중단."
#                 )
#                 self.precision_scan_failed = True
#                 return False
...
#             khj_xy = self.get_khj_scan_base_xy(color_str, local_id=local_id)
#             if khj_xy is None:
#                 self.get_logger().error(f"[PICK][KHJ] {color_str} KHJ 데이터 없음. 조립 중단.")
#                 self.precision_scan_failed = True
#                 return False
#             base_x, base_y = khj_xy
#             scan_x = base_x
...
#             scan_y = base_y + self.PRECISION_SCAN_GLOBAL_Y_OFFSET_M
#             self.get_logger().info(
#                 f"[PICK][KHJ] Z 하강({self.PRE_XY_LOWER:.1f}mm) → "
#                 f"XY({scan_x:.4f}, {scan_y:.4f})"
#             )
#             self.call(self.cli_r, GetTargetPose.Request(z=self.PRE_XY_LOWER, target_size="Z"))
#             self.call(self.cli_r, GetTargetPose.Request(x=scan_x, y=scan_y, z=0.0, target_size="XY"))
#             p = self.find_target_with_retry(color, enable_home_x_search=False, local_id=local_id)
#             if not p:
#                 self.get_logger().error(f"[PICK][KHJ] {color} 정밀 재촬영 실패. 조립 중단.")
#                 self.precision_scan_failed = True
#                 return False
...
#         if self.khj_valid:
#             # KHJ 유효: 처음에만 검증했으므로 재검증 없이 HOME 복귀 + 기준값 갱신만 한다.
#             self.get_logger().info(
#                 "[PICK] 파지 완료 (KHJ 유효). HOME 복귀 후 count 기준값 갱신."
#             )
#             self.update_floor_count_at_home(f"{color} 파지", before_count - 1)
#         else:
#             self.get_logger().info(
...
#                         scan_y = base_y + self.PRECISION_SCAN_GLOBAL_Y_OFFSET_M
#                         self.get_logger().info(
#                             f"[INSERT][KHJ] '{target_color}': "
#                             f"Z 하강 {self.PRE_XY_LOWER:.1f}mm → "
#                             f"XY ({scan_x:.4f}m, {scan_y:.4f}m)"
#                         )
#                         self.call(
#                             self.cli_r,
#                             GetTargetPose.Request(z=self.PRE_XY_LOWER, target_size="Z"),
...
#                     else:
#                         self.get_logger().warn(
#                             f"[INSERT][KHJ] '{target_color}' KHJ 데이터 없음. "
#                             "현재 위치에서 vision 시도."
#                         )
...
#                     if not p:
#                         self.get_logger().error(
#                             f"[INSERT][KHJ] {target_color} 정밀 재촬영 실패. 조립 중단."
#                         )
#                         self.precision_scan_failed = True
#                         return False
#                 elif pre_khj_scan:
#                     # KHJ 무효 + pre_khj_scan: KHJ 신뢰 불가이므로 vision_node만 사용
#                     self.get_logger().warn(
#                         f"[INSERT] khj_valid=False. pre_khj_scan 무시, "
#                         "vision_node로 직접 인식합니다."
#                     )
...
#             # khj_valid=True이거나 use_verify_pose이면 이미 스캔 완료 → 내부 precision scan 생략
#             skip_precision = use_verify_pose or (self.khj_valid and base_pose is None)
#             if not self.move_fast_from_pose(
#                 p,
#                 layer_index=layer_index,
...
#         self.get_logger().info("[INIT] HOME / robot2 assembly_joint / 그리퍼1,2 open 동시 시작")
#         for cli in (self.cli_h, self.cli_r2, self.cli_g, self.cli_g2):
#             while not cli.wait_for_service(timeout_sec=1.0):
#                 self.get_logger().info(f"Waiting for {cli.srv_name}...")
...
#         future_g2 = self.cli_g2.call_async(SetBool.Request(data=False))
...
#         rclpy.spin_until_future_complete(self, future_g2)
...
#             self.post_action_home_done = False
#             self.precision_scan_failed = False

#             action()

#             if self.precision_scan_failed:
#                 self.get_logger().error(
#                     "[FAIL] 정밀 재촬영 실패. robot1/2 동시 END 이동."
#                 )
#                 future_e1 = self.cli_r.call_async(GetTargetPose.Request(target_size="END"))
#                 future_e2 = self.cli_r2.call_async(GetTargetPose.Request(target_size="END"))
#                 rclpy.spin_until_future_complete(self, future_e1)
#                 rclpy.spin_until_future_complete(self, future_e2)
#             elif self.post_action_home_done:
#                 self.get_logger().info(
...
#         self.call(self.cli_g, SetBool.Request(data=False))
#         time.sleep(self.WAIT_TIME)
#         self.get_logger().info("[END] robot1/2 동시 END 이동")
#         future_e1 = self.cli_r.call_async(GetTargetPose.Request(target_size="END"))
#         future_e2 = self.cli_r2.call_async(GetTargetPose.Request(target_size="END"))
#         rclpy.spin_until_future_complete(self, future_e1)
#         rclpy.spin_until_future_complete(self, future_e2)
...
Z_OFF_DEFAULT        = -100.0   # 카메라 Z → 로봇 툴 Z 변환 오프셋 (mm)
Z_MARGIN_DEFAULT     =  23.0   # APPROACH 후 최종 하강 여유 거리 (mm)
BLOCK_H_DEFAULT      =  19.0   # 듀플로/레고 블록 한 층 높이 (mm)
WAIT_TIME_DEFAULT    =   0.7 # 모션 완료 후 대기 시간 (s)
PRE_XY_LOWER_DEFAULT = 60.0   # 정밀 재촬영을 위한 중간 Z 높이 (mm)
...
# 동작 전/후 바닥 블록 개수 검증 파라미터
# - ACTION_RETRY_MAX_DEFAULT: 같은 pick/insert 동작을 최대 몇 번까지 재시도할지
# - COUNT_VERIFY_TIMEOUT_SEC_DEFAULT: HOME 이동 후 block_count가 before-1로 바뀔 때까지 기다리는 시간
#   decision_assembly의 count_smooth_n이 크면 publish count가 늦게 바뀔 수 있으므로 즉시 실패시키지 않는다.
# - COUNT_VERIFY_POLL_SEC_DEFAULT: count topic 확인 주기
ACTION_RETRY_MAX_DEFAULT = 2
COUNT_VERIFY_TIMEOUT_SEC_DEFAULT = 5.0
COUNT_VERIFY_POLL_SEC_DEFAULT = 0.20
...
class MasterNode(Node):
    def __init__(self):
        ...
        self.PRECISION_SCAN_SETTLE_SEC = float(
            self.declare_parameter(
                "precision_scan_settle_sec",
                PRECISION_SCAN_SETTLE_SEC_DEFAULT,
            ).value
        )
        self.PRECISION_SCAN_SETTLE_SEC = max(0.0, self.PRECISION_SCAN_SETTLE_SEC)
        self.WRIST_OFFSET = float(
            self.declare_parameter("wrist_offset_deg", WRIST_OFFSET_DEFAULT).value
        )
        self.HOME_X_SEARCH_ENABLE = bool(
        ...
        self.ACTION_RETRY_MAX = int(
            self.declare_parameter(
                "action_retry_max",
                ACTION_RETRY_MAX_DEFAULT,
            ).value
        )
        self.ACTION_RETRY_MAX = max(1, self.ACTION_RETRY_MAX)

        self.COUNT_VERIFY_TIMEOUT_SEC = float(
            self.declare_parameter(
                "count_verify_timeout_sec",
                COUNT_VERIFY_TIMEOUT_SEC_DEFAULT,
            ).value
        )
        self.COUNT_VERIFY_TIMEOUT_SEC = max(0.5, self.COUNT_VERIFY_TIMEOUT_SEC)

        self.COUNT_VERIFY_POLL_SEC = float(
            self.declare_parameter(
                "count_verify_poll_sec",
                COUNT_VERIFY_POLL_SEC_DEFAULT,
            ).value
        )
        self.COUNT_VERIFY_POLL_SEC = max(0.05, self.COUNT_VERIFY_POLL_SEC)
        ...
        self.get_logger().info(
            ...
            "home_x_search_enable=%s, home_x_search_step_m=%.4fm, "
            "decision_block_count_topic=%s, "
            "action_retry_max=%d, count_verify_timeout_sec=%.2fs, "
            "count_verify_poll_sec=%.2fs"
            % (
                self.Z_OFF,
                self.Z_MARGIN,
                self.BLOCK_H,
                self.WAIT_TIME,
                self.PRE_XY_LOWER,
                self.PRECISION_SCAN_SETTLE_SEC,
                self.WRIST_OFFSET,
                self.PRECISION_SCAN_GLOBAL_Y_OFFSET_M,
                self.NEGATIVE_X_SCAN_EXTRA_ENABLE,
                self.NEGATIVE_X_EXTRA_GLOBAL_X_OFFSET_M,
                ...
                self.HOME_X_SEARCH_STEP_M,
                self.DECISION_BLOCK_COUNT_TOPIC,
                self.ACTION_RETRY_MAX,
                self.COUNT_VERIFY_TIMEOUT_SEC,
                self.COUNT_VERIFY_POLL_SEC,
            )
        )
    ...
    def wait_before_vision(self, label="vision"):
        """
        재촬영/탐색 위치로 이동한 직후 카메라 흔들림과 프레임 갱신을 기다린다.
        이 대기 이후 /get_target_pose 비전 서비스를 호출한다.
        """
        if self.PRECISION_SCAN_SETTLE_SEC <= 0.0:
            return

        self.get_logger().info(
            f"[VISION WAIT] {label}: {self.PRECISION_SCAN_SETTLE_SEC:.2f}s 대기 후 비전 요청"
        )
        time.sleep(self.PRECISION_SCAN_SETTLE_SEC)
    ...
    def verify_floor_count_drop_after_home(self, label, before_count):
        ...
        기존 코드는 HOME 이동 직후 처음 들어온 count 1개만 보고 바로 실패 처리했다.
        decision_assembly 쪽에서 rolling median(count_smooth_n)을 쓰면 실제 물체가 줄어도
        publish count가 몇 프레임 늦게 바뀔 수 있으므로, timeout 동안 before-1 값이 들어오는지
        기다린다.
        ...
        실패 조건:
            timeout 동안 expected_count가 들어오지 않음
            특히 after_count == before_count이면 "동작 실패/미집힘/미결합"으로 보고 상위 루프가 재시도한다.
        """
        if before_count is None:
            self.get_logger().error(f"[VERIFY][COUNT] {label} 이전 HOME count가 없습니다.")
            return False
        ...
        self.get_logger().info(
            f"[VERIFY][COUNT] {label}: HOME 이동 후 바닥 블록 개수 감소 확인 시작 "
            f"(before={before_count}, expected={expected_count}, "
            f"timeout={self.COUNT_VERIFY_TIMEOUT_SEC:.1f}s)"
        )
        ...
        # HOME 도착 이후 새로 publish된 count만 사용한다.
        after_time = time.monotonic()
        deadline = after_time + self.COUNT_VERIFY_TIMEOUT_SEC
        last_count = None
        last_stamp = None

        while rclpy.ok() and time.monotonic() < deadline:
            count = self.wait_decision_block_count(
                timeout_sec=self.COUNT_VERIFY_POLL_SEC,
                after_time=after_time,
            )
            ...
            count = int(count)
            last_count = count
            last_stamp = self.latest_decision_block_count_time

            self.get_logger().info(
                f"[VERIFY][COUNT] {label}: observed after_count={count}, "
                f"before={before_count}, expected={expected_count}"
            )

            if count == expected_count:
                self.current_floor_count_at_home = count
                self.current_insert_start_count = count
                self.current_insert_verify_after_time = None
                self.get_logger().info(
                    f"[VERIFY][COUNT] {label}: 바닥 블록 개수가 1개 줄었습니다. 정상으로 판단합니다."
                )
            ...
            # 같은 stamp를 반복해서 즉시 읽지 않도록 다음 loop에서는 새 topic만 기다린다.
            if last_stamp is not None:
                after_time = last_stamp + 1e-6
        ...
        if last_count == before_count:
            self.get_logger().warn(
                f"[VERIFY][COUNT] {label}: before={before_count}, after={last_count}. "
                "개수가 줄지 않았으므로 방금 동작 실패로 판단하고 재시도 대상입니다."
            )
        else:
            self.get_logger().warn(
                f"[VERIFY][COUNT] {label}: before={before_count}, after={last_count}, "
                f"expected={expected_count}. 예상과 다른 count라 실패로 판단합니다."
            )
    ...
    def find_target_with_retry(
        ...
        for x_offset in (-self.HOME_X_SEARCH_STEP_M, self.HOME_X_SEARCH_STEP_M):
            ...
            time.sleep(self.WAIT_TIME)
            self.wait_before_vision(f"HOME_X_SEARCH {color}")
    ...
    def move_fast_from_pose(
        ...
        if enable_precision_scan and original_class_name:
            ...
            self.wait_before_vision(f"PRECISION {original_class_name}")
    ...
    def pick_target(
        ...
        수정 핵심:
        - 파지 전 HOME에서 before_count를 읽는다.
        - 파지 후 HOME으로 복귀해 after_count가 before_count - 1인지 확인한다.
        - count가 그대로면 방금 파지가 실패한 것으로 보고 같은 pick 동작을 재시도한다.
        - KHJ 유효 경로에서도 더 이상 예상값으로 기준 count를 갱신하지 않고 실제 count로 검증한다.
        """
        ...
        while rclpy.ok():
            attempt += 1
            self.get_logger().info(
                f"[PICK ATTEMPT] {color} 파지 {attempt}회차"
            )
            ...
            if self.khj_valid:
                khj_xy = self.get_khj_scan_base_xy(color, local_id=local_id)
                if khj_xy is None:
                    self.get_logger().error(f"[PICK][KHJ] {color} KHJ 데이터 없음. 조립 중단.")
                    self.precision_scan_failed = True
                    return False
                ...
                self.wait_before_vision(f"PICK KHJ {color}")
            ...
            self.get_logger().info("[GRIPPER] close")
            self.call(self.cli_g, SetBool.Request(data=True))
            self.held_class = color
            time.sleep(0.1)

            self.get_logger().info(
                "[PICK] 파지 완료 후 HOME으로 이동해 decision_assembly block_count 1개 감소를 확인합니다."
            )

            if self.verify_floor_count_drop_after_home(f"{color} 파지", before_count):
                self.get_logger().info(
                    f"[PICK][COUNT] {color} 파지 성공: count {before_count} -> {before_count - 1}"
                )
                return True

            self.get_logger().warn(
                f"[PICK][COUNT] {color} 파지 검증 실패 ({attempt}회차). 재시도합니다."
            )

            # 실패 시 다음 시도를 위해 그리퍼를 열고 held 상태를 초기화한다.
            self.call(self.cli_g, SetBool.Request(data=False))
            self.held_class = None
            time.sleep(self.WAIT_TIME)
    ...
    def visual_insert(
        ...
        attempt = 1
        retry_pose = None
        gripper_opened_early = False   # open_after_insert로 이미 열린 경우
        while rclpy.ok():
            self.get_logger().info(
                f"[INSERT ATTEMPT] {target_color} visual_insert {attempt}회차"
            )
            ...
            # 그리퍼가 이미 열린 상태(open_after_insert 사용 후 검증 실패)이면
            # 재삽입 불가 → 재검증만 시도한다.
            if gripper_opened_early:
                self.get_logger().warn(
                    f"[INSERT][RETRY] 그리퍼 오픈 후 검증 실패 ({attempt}회차). "
                    "재삽입 없이 재검증합니다."
                )
                if self.verify_insert_from_saved_scan(target_color, home_after_success=True):
                    if release_gripper:
                        self.held_class = None
                    else:
                        self.held_class = held_before_insert
            ...
            if use_verify_pose:
                ...
                if self.khj_valid:
                    # KHJ 유효: KHJ 위치로 이동 후 정밀 재촬영
                    # khj_scan_class가 지정된 경우 해당 클래스의 KHJ 위치로 이동
                    nav_class = khj_scan_class if khj_scan_class else target_color
                    khj_xy = self.get_khj_scan_base_xy(nav_class, local_id=local_id)
                    if khj_xy is not None:
                        base_x, base_y = khj_xy
                        scan_x = base_x
                        if self.NEGATIVE_X_SCAN_EXTRA_ENABLE and base_x > 0.0:
                        ...
                        scan_y = base_y + self.PRECISION_SCAN_GLOBAL_Y_OFFSET_M
                        self.get_logger().info(
                            f"[INSERT][KHJ] '{target_color}' 탐색 위치: '{nav_class}' KHJ → "
                            f"Z 하강 {self.PRE_XY_LOWER:.1f}mm → "
                            f"XY ({scan_x:.4f}m, {scan_y:.4f}m)"
                        )
                        self.call(
                        ...
                        self.wait_before_vision(f"INSERT KHJ {target_color}")
                    else:
                        self.get_logger().warn(
                            f"[INSERT][KHJ] '{nav_class}' KHJ 데이터 없음. "
                            "현재 위치에서 vision 시도."
                        )
                    p = self.find_target_with_retry(
                        target_color,
            ...
            if open_after_insert:
                self.get_logger().info("[GRIPPER] 삽입 직후 그리퍼 오픈 (홈 이동 전)")
                self.call(self.cli_g, SetBool.Request(data=False))
                gripper_opened_early = True
            elif release_gripper:
                self.get_logger().info(
                    "[GRIPPER] release_gripper=True이지만 검증 전이므로 open하지 않습니다. "
                    "그리퍼를 닫은 상태로 block_count 검증까지 유지합니다."
                )
    ...
    def build_burger(self):
        ...
        self.get_logger().info("[Phase 1] 4x2_yellow 파지 (베이스 가까운 쪽, local_id=0)")
        if not self.pick_target("4x2_yellow", local_id=0):
            self.get_logger().warn("4x2_yellow 파지 실패. 조립 취소.")
            return
        ...
        self.get_logger().info(
            "[Phase 4] KHJ 스캔 위치 이동 후 4x2_yellow 비전 → 최종 결합 (두 번째 블록, local_id=1)"
        )
        if self.visual_insert(
            "4x2_yellow",
            layer_index=1.4,
    ...
    def run(self):
        ...
        while rclpy.ok():
            ...
            self.get_logger().info(f"작업 시작: {user_input} (Ctrl+C로 중단)")
            attempt = 0
            while rclpy.ok():
                attempt += 1
                self.assembly_completed = False
                ...
                self.get_logger().info(f"[시도 {attempt}회차] {user_input} 조립 시작")
                action()

                if self.assembly_completed:
                    self.get_logger().info(f"[성공] {attempt}회차 조립 완료")
                    break

                if self.precision_scan_failed:
                    self.get_logger().warn(
                        f"[실패 {attempt}회차] 정밀 재촬영 실패. END 이동 후 재시도..."
                    )
                    future_e1 = self.cli_r.call_async(GetTargetPose.Request(target_size="END"))
                    future_e2 = self.cli_r2.call_async(GetTargetPose.Request(target_size="END"))
                    rclpy.spin_until_future_complete(self, future_e1)
                    ...
                    self.get_logger().warn(
                        f"[실패 {attempt}회차] 조립 실패. HOME 이동 후 재시도..."
                    )
                    self.call(self.cli_h, Trigger.Request())
                time.sleep(1.0)
            ...
            if self.post_action_home_done:
                ...
                self.get_logger().info("[POST] 안전하게 HOME 이동")
                self.call(self.cli_h, Trigger.Request())
                time.sleep(1.0)
